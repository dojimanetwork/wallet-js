...
export async function priceConvertor(
  api: string,
  amount: number,
  fromAsset: AssetsList,
  toAsset: AssetsList
) {
  ...
  if (response.status !== 200) {
    throw new Error(`Unable to retrieve prices from the endpoint.`);
  }
...
export async function getAssets(api: string) {
  ...
  if (response.status !== 200) {
    throw new Error(`Unable to retrieve assets list from the endpoint.`);
  }
