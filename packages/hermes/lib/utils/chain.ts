 ...
 */
export const isChain = (c: string): c is Chain =>
  (Object.values(Chain) as string[]).includes(c);
 ...
 */
export const chainToString: ((chainId: Chain) => string) &
  Record<Chain, string> = Object.assign(
  (chainId: Chain) => {
 ...
 */
export const isHermesChain = (chain: Chain): boolean =>
  eqChain(chain, HermesChain);
