  ...
  switch (network) {
    ...
    case Network.DojTestnet: {
      ...
      return await Promise.all(
        utxos.map(async (utxo) => ({
          ...
          txHex: withTxHex
            ? await getTxHex({ txHash: utxo.txid, sochainUrl, network })
            : undefined,
        }))
      );
    }
    ...
    case Network.Stagenet: {
      ...
      const utxos: HaskoinApiTypes.UtxoData[] = confirmedOnly
        ? await haskoinApi.getConfirmedUnspentTxs({
            address,
            haskoinUrl,
            sochainUrl,
            network,
...
export const buildTx = async ({
  ...
  if (utxos.length === 0)
    throw new Error(
      "No confirmed UTXOs. Please wait until your balance has been confirmed on-chain."
    );
 ...
 */
export const haskoinbroadcastTx = async ({
  haskoinUrl,
  txHex,
...
export const sochainbroadcastTx = async ({
  sochainUrl,
  txHex,
  network,
}: TxBroadcastParams): Promise<TxHash> => {
