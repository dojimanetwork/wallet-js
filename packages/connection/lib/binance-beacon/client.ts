 ...
 */
class BinanceBeaconClient
  extends BaseChainClient
  implements BinanceClient, ChainClient
{
  ...
  async getAccount(address?: Address, index = 0): Promise<Account> {
    ...
    if (!response || !response.result || !isAccount(response.result))
      return Promise.reject(
        Error(`Could not get account data for address ${accountAddress}`)
      );
  ...
  async getBalance(address: Address, assets?: Asset[]): Promise<Balance[]> {
    if (this.network === Network.Testnet) {
      ...
      const balances: BinanceBalance[] = await this.bncClient.getBalance(
        address
      );
   ...
   * */
  private async searchTransactions(params?: {
      ...
      if (value) {
        ...
        if (key === "startTime" && !params["endTime"]) {
          url.searchParams.set(
            "endTime",
            (parseInt(value) + diffTime).toString()
          );
        }
  ...
  async multiSend({
    ...
    transactions,
    memo = "",
  }: MultiSendParams): Promise<TxHash> {
  ...
  private async getTransferFee(): Promise<TransferFee> {
    const feesArray = (
      await axios.get<BinanceFees>(`${this.getClientUrl()}/api/v1/fees`)
    ).data;
  ...
  async getFees(): Promise<Fees> {
    if (this.network === Network.Testnet) {
      ...
      try {
        ...
        console.log(error);
        console.warn(
          `Error pulling rates from hermeschain, will try alternate`
        );
      }
      if (!singleTxFee) {
