...
import {DRC20} from "../dojimachain/DRC20.sol";
...
interface iROUTER {
    ...
    function switchWithExpiry(address, address, string calldata, uint) external;
}

// Router is managed by Hermes Chain Vaults
contract Router is Ownable, ReentrancyGuard {
    address public DOJ_TOKEN;
    error InvalidAsset();
    ...
    // Instance of the DRC20 contract
    DRC20 private dojimaToken;
    ...
    // Emitted for all switch deposits, the memo distinguishes for swap, add, remove, donate etc
    event Switch(address indexed to, address indexed asset, uint amount, string memo);
    ...
    function initialize(address _dojToken) external {
    ...
    function switchWithExpiry(address payable vault, address asset, string memory memo, uint expiration) external payable {
        require(block.timestamp < expiration, "Switch: expired");
        _switch(vault, asset, memo);
    }

    // Deposit an asset with a memo. Only Dojima Native is forwarded
    ...
    // Switch an asset with a memo. Only Dojima Native is forwarded
    function _switch(address payable vault, address asset, string memory memo) private nonReentrant{
        uint _safeAmount;
        if (asset == DOJ_TOKEN) {
            _safeAmount = msg.value;
            DRC20(DOJ_TOKEN).withdrawDoj{value: _safeAmount}();
        } else {
            revert InvalidAsset();
        }

        emit Switch(vault, asset, _safeAmount, memo);
    }
    ...
    // Note: Contract recipients of Dojima Native are only given 2300 Gas to complete execution.
    function transferOut(address payable to, address asset, uint amount, string memory memo) public payable nonReentrant {
        uint _safeAmount;
        if(asset == DOJ_TOKEN) {
            _safeAmount = msg.value;
            bool success = to.send(_safeAmount); // Send Dojima Native.
            if (!success) {
