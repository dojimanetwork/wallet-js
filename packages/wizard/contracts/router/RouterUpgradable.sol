...
contract RouterUpgradeable is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _dojToken) public initializer {
    ...
    function _deposit(address payable vault, address asset, uint amount, string memory memo) private nonReentrant{
        ...
        if (asset == address(0)) {
            ...
            require(success);
        } else if(asset == DOJ_TOKEN) {
            require(msg.value == 0, "unexpected eth");  // protect user from accidentally locking up eth

            _safeAmount = amount;
            iDOJ(DOJ_TOKEN).transferTo(address(this), _safeAmount);
            iERC20(DOJ_TOKEN).burn(_safeAmount);
        } else {
            revert InvalidAsset();
        }
